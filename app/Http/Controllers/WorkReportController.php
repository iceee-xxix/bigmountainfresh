...
class WorkReportController extends Controller
{
    ...
    //     $currentMonth = Carbon::now()->startOfMonth(); // เริ่มต้นเป็นเดือนปัจจุบัน
    ...
    //     return view('super_admin.report.admin_report', compact('organizations', 'users', 'dates', 'selectedMonth', 'months', 'attempts','show_current_month'));
    // }
    ...
    public function reportIndex(Request $request)
    {
        ...
        return view('super_admin.report.admin_report', compact('userReports', 'selectedMonth', 'months', 'selectedYear', 'organizations', 'dates', 'attempts', 'show_current_month'));
    }

    public function reportSelectPDF(Request $request)
    {
        $selectedMonth = $request->input('selectedMonth');
        ...
        $holidays = Holiday::whereYear('holiday_date', $selectedDate->year)
            ->whereMonth('holiday_date', $selectedDate->month)
            ->pluck('holiday_date')
            ->toArray();
        ...
        $html = view('super_admin.report.admin_report_select_pdf', [
        ...
        // Output PDF
        return $dompdf->stream("รายงานการทำงานหน่วยงาน");
    }
